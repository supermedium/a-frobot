...
function bumpAframeDist (data) {
  ...
  return new Promise(resolve => {
    ...
    async.series([
      ...
      execCommand('git add -A .', 'aframe'),
      execCommand(`git commit -m "Bump aframe-master dist/ builds. (${data.compare})"`,
                  'aframe')
    ], err => {
      // Bump CDN URLs.
      if (process.env.AFROBOT_ENV !== 'test') { process.chdir('./aframe'); }
      gitRev.long(hash => {
        if (process.env.AFROBOT_ENV !== 'test') { process.chdir('../'); }
        async.series([
