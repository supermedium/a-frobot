  ...
  return new Promise(resolve => {
    ...
    gitRev.short(hash => {
      async.series([
        ...
        cb => replacePackageJsonVersion(cb),
...
/**
 * Replace package.json version field with new hash.
 */
function replacePackageJsonVersion (cb) {
  gitRev.short(hash => {
    fs.readFile('aframe/package.json', 'utf-8', (err, data) => {
      fs.writeFile('aframe/package.json', replaceVersion(data, hash), cb);
    });
  });
}
 ...
 */
function replaceReadmeCdnUrls (cb) {
  gitRev.short(hash => {
...
/**
 * "0.4.0-#abc123" to "0.4.0-#def456".
 */
function replaceVersion (data, hash) {
  return data.replace(/"version": "(\d+.\d+.\d+).*"/, `"version": "$1-#${hash}"`);
}
