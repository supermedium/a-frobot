...
function bumpAframeDocs (data) {
  ...
  return new Promise(resolve => {
    ...
    async.series([
      ...
      execCommand(
        'node --max-old-space-size=100 /app/.heroku/node/bin/npm install --only="dev"',
        'aframe-site'),
      execCommand('node --max-old-space-size=100 /app/.heroku/node/bin/npm install',
                  'aframe-site'),
      execCommand(
        'MULTIDEP_NPM_PATH=/app/.heroku/node/bin/npm MULTIDEP_MEMORY_LIMIT=100 ' +
